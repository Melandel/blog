...
**Vim**. You may have heard about it - it is, after all, [_ubiquitous_](https://www.vim.org/). Most people scratch the surface, find it too unintuitive, and walk away. Some others stay, but don't dig. _I_ dug for you.
...
| "Yank (copy) until the beginning of the 2nd next word." | `y2w`       | `Yank`        | `until the beginning of the next word` | `2`           |
...
[![repeatlastchange]({{ site.baseurl }}/assets/img/repeatlastchange.gif)]({{ site.baseurl }}/assets/img/repeatlastchange.gif)
...
| `>` Increase indentation    | `t[c]` till before character `[c]` in the line      | `gg` move to the first line of the file                                                |
| `<` Decrease indentation    | on all visually selected characters                 | `G` move to the last line of the file                                                  |
| `r` Replace                 | `:g/regex/cmd` on every line matching `/regex/`     | `w` move to the beginning of next word                                                 |
| `J` Join line with the next | `:v/regex/cmd` on every line NOT matching `/regex/` | `}` move to the end of the current paragraph                                           |
| `A` Append to line          | `aw` on a word                                      | `:cn` move to a position described by the output of a program (_eg_ compilation error) |
| `I` Prepend to line         | `a{` on a block defined by brackets                 | `gd` move to local declaration (of a function, for instance)                           |
...
For the sake of **curiosity**, here is a bigger sample of vim language elements you could come across:
...
| `!` Filter through an external program       | `aw` on a word                                       | `w` move to the beginning of next word                                                 |
...
| `>` Increase indentation                     | `aa` on a function argument                          | `n` move to the beginning of a searched regex                                          |
| `<` Decrease indentation                     | on all visually selected characters                  | `n` move to the end of a searched regex                                                |
| `zf` Define a fold                           | on all visually selected lines                       | `gf` move to the file which path is under the cursor                                   |
| `ga` Vertically align                        | `:g/regex/cmd` on every line matching `/regex/`      | `hjkl` move n'th characters to the left, right, top or down of current position        |
| `r` Replace                                  | `:v/regex/cmd` on every line NOT matching `/regex/`  | `<C-F>` move n'th "pages" down of current position                                     |
| `J` Join line with the next                  | `:%[cmd]` on every line of the file                  | `"[m]` move to a position you have assigned to a marker in the past                    |
| `S` Split line by a given character          | `)` till next sentence                               | `<C-O>` move to a position you have jumped from in the past                            |
| `@` Repeat a macro                           | `al` on a line                                       | `g;` move to a position you have edited in the past                                    |
